"""Tool definitions and implementations for dev-council."""
import json
import os
import re
...
# A direct prompt path is used so the agent can ask focused questions without
# depending on a background REPL question queue.
...
def _ask_user_question(
    ...
    print()
    print("\033[1;35mQuestion from assistant:\033[0m")
    print(f"  {question}")
    ...
    if options:
        ...
        while True:
            raw = ask_input_interactive("Your choice (number or text): ", {}).strip()
            if not raw:
                return ""
            if raw.isdigit():
                selection = int(raw)
                if 1 <= selection <= len(options):
                    return options[selection - 1].get("label", "")
                if selection == 0 and allow_freetext:
                    return ask_input_interactive("Your answer: ", {}).strip()
...
def _sleeptimer(seconds: int, config: dict) -> str:
    ...
    if not cb:
        return "Error: Internal callback missing, dev-council did not provide _run_query_callback"
...
# Reserved section removed: no multi-agent support in dev-council.
...
# ── Plugin tools ───────────────────────────────────────────────────────────────
# Reserved section removed: no plugin loading in dev-council.
...
def _enter_plan_mode(params: dict, config: dict) -> str:
    ...
    plans_dir = Path.cwd() / ".dev-council" / "plans"
