...
Storage layout:
  user scope    : ~/.dev-council/memory/<slug>.md
  project scope : .dev-council/memory/<slug>.md  (relative to cwd)
...
USER_MEMORY_DIR = CONFIG_DIR / "memory"
...
def get_project_memory_dir() -> Path:
    ...
    return Path.cwd() / ".dev-council" / "memory"
...
def get_memory_dir(scope: str = "user") -> Path:
    ...
    Args:
        scope: "user" (global ~/.dev-council/memory) or
               "project" (.dev-council/memory relative to cwd)
    """
    if scope == "project":
